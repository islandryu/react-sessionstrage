...
export default function useSessionStorage(dependencies) {
  ...
  const [sessionStorages, setSessionStorages] = useState(
    SessionStoragesObj.getAll()
  );
  useEffect(() => {
    function onChange() {
  ...
  const setSessionStorage = useMemo(
    () => SessionStoragesObj.setItem.bind(SessionStoragesObj),
    [sessionStorages]
  );
  const removeSessionStorage = useMemo(
    () => SessionStoragesObj.removeItem.bind(SessionStoragesObj),
    [sessionStorages]
  );
